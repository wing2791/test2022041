...
//            传染邻近格数
        ...
        //        第0行：人的Id
        //        第1行：是否感染，0：未感染 1：感染
        //        第2行：地点 0：图书馆 1：宿舍（默认所有人在同一个宿舍）2：食堂 3：大教室1 4：大教室2 5：小教室1 6：小教室2
        ...
        //        大教室的人数
        int bigClassContainer = 120;
        //        大教室数量
        int bigClassNumber = 2;
        //        小教室容量
        int smallClassContainer = 80;
        //        小教室数量
        ...
        int chuanRanGirdNumber_BigClass1 = 8;
        double ganRan_Probability_BigClass1 = 0.3;
        int chuanRanGirdNumber_BigClass2 = 8;
        double ganRan_Probability_BigClass2 = 0.3;
        int chuanRanGirdNumber_SmallClass1 = 6;
        double ganRan_Probability_SmallClass1 = 0.4;
        int chuanRanGirdNumber_SmallClass2 = 6;
        double ganRan_Probability_SmallClass2 = 0.4;
        int chuanRanGirdNumber_library = 10;
        double ganRan_Probability_library = 0.2;
        int chuanRanGirdNumber_canteen = 4;
        double ganRan_Probability_canteen = 0.8;
        int chuanRanGirdNumber_dormitory = 5;
        double ganRan_Probability_dormitory = 0.5;
...
//                ganRan_Probability = Double.parseDouble(scann.next());
                ...
                if (ganRan_Probability <= 1 && ganRan_Probability >= 0 && sign>=0) break;
            } catch (Exception e) {
...
//            小教室2
    ...
    //            前240大教室，后160小教室，由于studyPersonID是随机的，相当于400人随机找教室上课
                if (i < 120) {
...
//                        查看是否有感染能力
                        if(!ganRanPersonsIdLibrary.contains(ganRanPersonsId.get(i)))
                            ganRanPersonsIdLibrary.add(ganRanPersonsId.get(i));
                        if(personStateInfo[4][ganRanId]>=chuanBo_Day  && rd.nextDouble()<=ganRan_Probability_library){
//                            具有感染能力
//                        获取感染人的索引
                            int ganRanIndex = location.indexOf(ganRanId);
//                            感染周围人
                            int[] ganRanList = GanRanList(location,ganRanIndex,chuanRanGirdNumber_library);
...
//                        查看是否有感染能力
                        if(!ganRanPersonsIdDormitory.contains(ganRanPersonsId.get(i)))
                            ganRanPersonsIdDormitory.add(ganRanPersonsId.get(i));
                        if(personStateInfo[4][ganRanId]>=chuanBo_Day && rd.nextDouble()<=ganRan_Probability_dormitory){
//                            具有感染能力
//                        获取感染人的索引
                            int ganRanIndex = location.indexOf(ganRanId);
//                            感染周围人
                            int[] ganRanList = GanRanList(location,ganRanIndex,chuanRanGirdNumber_dormitory);
                            for(int j = 0;j<ganRanList.length;j++){
//                                周围的人全部感染
...
//                        查看是否有感染能力
                        if(!ganRanPersonsIdCanteen.contains(ganRanPersonsId.get(i)))
                            ganRanPersonsIdCanteen.add(ganRanPersonsId.get(i));
                        if(personStateInfo[4][ganRanId]>=chuanBo_Day && rd.nextDouble()<=ganRan_Probability_canteen){
//                            具有感染能力
//                        获取感染人的索引
                            int ganRanIndex = location.indexOf(ganRanId);
//                            感染周围人
                            int[] ganRanList = GanRanList(location,ganRanIndex,chuanRanGirdNumber_canteen);
                            for(int j = 0;j<ganRanList.length;j++){
//                                周围的人全部感染
...
//                        查看是否有感染能力
                        if(!ganRanPersonsIdBigClass1.contains(ganRanPersonsId.get(i)))
                            ganRanPersonsIdBigClass1.add(ganRanPersonsId.get(i));
                        if(personStateInfo[4][ganRanId]>=chuanBo_Day && rd.nextDouble()<=ganRan_Probability_BigClass1){
//                            具有感染能力
//                        获取感染人的索引
                            int ganRanIndex = location.indexOf(ganRanId);
//                            感染周围人
                            int[] ganRanList = GanRanList(location,ganRanIndex,chuanRanGirdNumber_BigClass1);
                            for(int j = 0;j<ganRanList.length;j++){
...
//                        查看是否有感染能力
                        if(!ganRanPersonsIdBigClass2.contains(ganRanPersonsId.get(i)))
                            ganRanPersonsIdBigClass2.add(ganRanPersonsId.get(i));
                        if(personStateInfo[4][ganRanId]>=chuanBo_Day && rd.nextDouble()<=ganRan_Probability_BigClass2){
//                            具有感染能力
//                        获取感染人的索引
                            int ganRanIndex = location.indexOf(ganRanId);
//                            感染周围人
                            int[] ganRanList = GanRanList(location,ganRanIndex,chuanRanGirdNumber_BigClass2);
                            for(int j = 0;j<ganRanList.length;j++){
//                                周围的人全部感染
...
//                    如果小教室1有感染人
                    ...
                    if(location.contains(ganRanPersonsId.get(i))){
//                        查看是否有感染能力
                        if(!ganRanPersonsIdSmallClass1.contains(ganRanPersonsId.get(i)))
                                ganRanPersonsIdSmallClass1.add(ganRanPersonsId.get(i));
                        if(personStateInfo[4][ganRanId]>=chuanBo_Day && rd.nextDouble()<=ganRan_Probability_SmallClass1){
//                            具有感染能力
//                        获取感染人的索引
                            int ganRanIndex = location.indexOf(ganRanId);
//                            感染周围人
                            int[] ganRanList = GanRanList(location,ganRanIndex,chuanRanGirdNumber_SmallClass1);
                            for(int j = 0;j<ganRanList.length;j++){
//                                周围的人全部感染
...
//                        查看是否有感染能力
                        if(!ganRanPersonsIdSmallClass2.contains(ganRanPersonsId.get(i)))
                            ganRanPersonsIdSmallClass2.add(ganRanPersonsId.get(i));
                        if(personStateInfo[4][ganRanId]>=chuanBo_Day && rd.nextDouble()<=ganRan_Probability_SmallClass2){
//                            具有感染能力
//                        获取感染人的索引
                            int ganRanIndex = location.indexOf(ganRanId);
//                            感染周围人
                            int[] ganRanList = GanRanList(location,ganRanIndex,chuanRanGirdNumber_SmallClass2);
                            for(int j = 0;j<ganRanList.length;j++){
//                                周围的人全部感染
...
//            System.out.println("\t天数"+"\t总感染："+"\t图书馆："+"\t宿舍："+"\t食堂："+"\t大教室1："+"\t大教室2："+"\t大教室3："+"\t小教室1："+"\t小教室2：");
//
            System.out.println("\t天数"+(Math.floorDiv(sign,3)+1)+"\t总感染："+ganRanPersonsId.size()+"\t图书馆："+ganRanPersonsIdLibrary.size()+"\t宿舍："+ganRanPersonsIdDormitory.size()+"\t食堂："+ganRanPersonsIdCanteen.size()+"\t大教室1："+ganRanPersonsIdBigClass1.size()+"\t大教室2："+ganRanPersonsIdBigClass2.size()+"\t小教室1："+ganRanPersonsIdSmallClass1.size()+"\t小教室2："+ganRanPersonsIdSmallClass2.size());
            sign++;
//            if(sign%3 == 0) day++;
        }
